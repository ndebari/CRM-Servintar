...
## Corrección de detección en el corredor TRP–Terminal Zárate (17/09/2026)

La distancia a un nodo no distingue una cabina de acceso lateral de la autopista paralela. `netlify/lib/toll-station-details.json` agrega identidad de plaza, concesionaria y trazado de acceso para los puntos revisados de Illia, Retiro II, Salguero, Sarmiento y Campana. Los accesos requieren coincidencia con su vía a 20 y 40 metros del punto de cobro, en el sentido y la pasada actuales, con tolerancia de 8 m. No se eliminan: siguen detectándose al usar realmente esa entrada. Las plazas identificadas mantienen un ID estable entre carriles, no se fusionan con otra plaza identificada y conservan cruces en sentido contrario. Los puntos indicados como próximos a inaugurarse quedan excluidos.

Fuentes: [identidades y ubicaciones de AUSA](https://data.buenosaires.gob.ar/dataset/peajes-porticos-autopistas), [conversión de Illia a pórticos](https://buenosaires.gob.ar/gcaba_historico/noticias/illia-se-habilito-la-autopista-inteligente-en-ambos-sentidos), [AUSOL](https://www.ausol.com.ar/valores), y geometría vial OpenStreetMap consultada mediante OSRM. Retiro II y el pórtico troncal Illia comparten identidad de plaza para evitar duplicar el mismo cobro tras la reconversión.

La prueba de referencia `tests/fixtures/illia-zarate.json` conserva un trazado OSRM de ida y vuelta por este corredor: Retiro II/Campana a la ida y Campana/Illia a la vuelta. No es el trazado exacto de Google del usuario ni certifica circulación de camiones. La corrección de accesos tiene cobertura explícita de estos puntos; el resto del catálogo mantiene la detección aproximada anterior y necesita revisión. Validación: 36 pruebas, TypeScript y bundle directo con esbuild. El sitio publicado se comprobó antes de la corrección: devolvió seis pasos para TRP–Terminal Zárate, incluyendo dos candidatos de accesos próximos. La clave de Google no permite probar Directions desde localhost.
