            ...
            {getTollGroups(draft).map(group => (
              ...
              {group.rows.map(({ toll, index }, groupIndex) => (
                <div className="toll-card" key={toll.id}>
                  ...
                  <div className="toll-card-amount"><label>Importe (ARS)<input aria-label={'Importe del peaje ' + (index + 1)} type="number" min="0" step="0.01" placeholder="Completar tarifa" value={toll.amount ?? ''} onChange={event => updateDraft('tolls', draft.tolls.map(item => item.id === toll.id ? { ...item, amount: event.target.value === '' ? null : Math.max(0, Number(event.target.value)), source: 'manual' } : item))} /></label></div>
                </div>
              ))}
                </div>
      ...
      <section className="client-detail-layout">
        <div className="panel">
          ...
          <div className="form-grid">
            ...
            <label>
              Razón social
              <input value={editingClient.businessName} onChange={(event) => { nameRevision.current += 1; setEditingClient({ ...editingClient, businessName: event.target.value }); }} />
            </label>
            <label>
              Nombre de fantasía
